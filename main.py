...
@app.get("/Restaurant/{restaurant_id}", response_model=RestaurantIn_Pydantic, responses={404: {"model": HTTPNotFoundError}})
...
@app.get("/panier", response_model=List[Panier_Pydantic], status_code=200)
async def return_all_panier():
    return await Panier_Pydantic.from_queryset(Panier.all())

#post panier

@app.post("/panier", response_model=PanierIn_Pydantic, status_code=status.HTTP_201_CREATED)
async def add_panier(panier: PanierIn_Pydantic):
    obj = await Panier.create(**panier.dict(exclude_unset=True))
    return await Panier_Pydantic.from_tortoise_orm(obj)



# get category

@app.get("/category", response_model=List[Category_Pydantic], status_code=200)
async def return_all_category():
    return await Category_Pydantic.from_queryset(Category.all())

#post category

@app.post("/category/add_category", response_model=CategoryIn_Pydantic, status_code=status.HTTP_201_CREATED)
async def add_category(category: CategoryIn_Pydantic):
    obj = await Category.create(**category.dict(exclude_unset=True))
    return await Category_Pydantic.from_tortoise_orm(obj)


# get Post ProduitPanier
@app.post("/produit_panier", response_model=ProduitPanierIn_Pydantic)
async def create_produit_panier(produi_panier: ProduitPanierIn_Pydantic):
    obj = await ProduitPanier.create(**produi_panier.dict(exclude_unset=True))
    return await ProduitPanierIn_Pydantic.from_tortoise_orm(obj)



# get all user
@app.get("/produit_paniers", response_model=List[ProduitPanier_Pydantic])
async def get_all_users():
    return await ProduitPanier_Pydantic.from_queryset(ProduitPanier.all())



## Les requettes

# Add resquests for commande
@app.post("/commande", response_model=CommandeIn_Pydantic)
async def create_commande(commande: CommandeIn_Pydantic):
    obj = await Commande.create(**commande.dict(exclude_unset=True))
    return await CommandeIn_Pydantic.from_tortoise_orm(obj)

# get all repas
@app.get("/commandes/", response_model=List[Commande_Pydantic])
async def get_all_commandes():
    return await Commande_Pydantic.from_queryset(Commande.all())
...
register_tortoise(
    app,
    db_url="postgres://postgres:tonystark@localhost/Restaurant",
